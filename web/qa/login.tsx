...
function Preview() {
  ...
  return <>
    <LoginPage checking={state === 'loading'} configured={state !== 'unconfigured'} widgetError={state === 'error'} onRetry={() => setState('ready')}>
    ...
    <details style={{position:'fixed',bottom:8,right:8,zIndex:6000,fontSize:11,background:'#fff',color:'#142b49',border:'1px solid #dce4ee',borderRadius:7,padding:'7px 10px'}}>
      ...
      <label>Состояние входа <select value={state} onChange={event => setState(event.target.value)}><option value="ready">Готов к входу</option><option value="loading">Проверка авторизации</option><option value="error">Ошибка виджета</option><option value="unconfigured">Вход не настроен</option></select></label>
    </details>
  </>
}
